...
for sfile in skins:
    ...
    try:
        ...
        if h * 2 == w:
            ...
            # Leg
            imo.paste(imi.crop(rl_top).transpose(
                Image.FLIP_LEFT_RIGHT), ll_top)
            imo.paste(imi.crop(rl_btm).transpose(
                Image.FLIP_LEFT_RIGHT), ll_btm)
            imo.paste(imi.crop(rl_ins).transpose(
                Image.FLIP_LEFT_RIGHT), ll_out)
            imo.paste(imi.crop(rl_out).transpose(
                Image.FLIP_LEFT_RIGHT), ll_ins)
            imo.paste(imi.crop(rl_fro).transpose(
                Image.FLIP_LEFT_RIGHT), ll_fro)
            imo.paste(imi.crop(rl_bak).transpose(
                Image.FLIP_LEFT_RIGHT), ll_bak)
            # Arm
            imo.paste(imi.crop(ra_top).transpose(
                Image.FLIP_LEFT_RIGHT), la_top)
            imo.paste(imi.crop(ra_btm).transpose(
                Image.FLIP_LEFT_RIGHT), la_btm)
            imo.paste(imi.crop(ra_ins).transpose(
                Image.FLIP_LEFT_RIGHT), la_out)
            imo.paste(imi.crop(ra_out).transpose(
                Image.FLIP_LEFT_RIGHT), la_ins)
            imo.paste(imi.crop(ra_fro).transpose(
                Image.FLIP_LEFT_RIGHT), la_fro)
            imo.paste(imi.crop(ra_bak).transpose(
                Image.FLIP_LEFT_RIGHT), la_bak)
            imo.save(sfile)
            success += 1
        else:
            failure += 1
